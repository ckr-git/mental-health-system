...
**最后更新**: 2026-03-14 | **版本**: v2.0 (Phase 1-6 完成)
...
| 类别 | 数量 | 说明 |
|------|------|------|
| Java 类文件 | 204 | 后端业务逻辑实现 |
| Entity 实体类 | 40 | 数据模型定义 |
| Controller 控制器 | 30 | REST API 端点 |
| Service 服务类 | 35 | 核心业务逻辑 |
| Mapper 接口 | 40 | MyBatis-Plus 数据访问层 |
| DTO 类 | 15 | 数据传输对象 |
| Vue 页面 | 38 | 患者15 + 医生11 + 管理员8 + 公共3 + 布局3 |
| Vue 组件 | 25 | 可复用 UI 组件 |
| Pinia Store | 4 | 状态管理 (user/assessment/notification/profile) |
| 数据库表 | 35+ | 核心业务 + Phase 1-6 新增表 |
| Flyway 迁移 | 8 | 数据库版本管理 |
| API 接口 | 150+ | RESTful API 端点 |
...
| 命令 | 说明 |
|------|------|
| `mvn clean verify` | 编译 + 运行测试 |
| `mvn spring-boot:run` | 启动后端开发服务器 (端口 8080) |
| `cd frontend && npm install` | 安装前端依赖 |
| `cd frontend && npm run dev` | 启动前端开发服务器 (Vite) |
| `cd frontend && npm run build` | 生产构建 (vue-tsc 类型检查 + Vite 打包) |
| `cd frontend && npm run preview` | 预览生产构建结果 |
| `scripts/start-backend.bat` | 运行后端可执行 Jar |
| `scripts/start.bat` | 启动服务 |
| `scripts/stop.bat` | 停止服务 |
| `scripts/init-database.bat` | 初始化数据库 |
| `scripts/import-test-accounts.bat` | 导入测试账号 |
| `powershell -File scripts/test-all-apis.ps1` | REST API 冒烟测试 (60+ 接口) |
<!-- AUTO-GENERATED: end -->
...
`src/main/resources/application.yml` 支持通过环境变量覆盖敏感配置：

<!-- AUTO-GENERATED: environment-variables -->
| 变量 | 必需 | 说明 | 默认值 |
|------|------|------|--------|
| `DB_HOST` | 否 | MySQL 主机地址 | `localhost` |
| `DB_PORT` | 否 | MySQL 端口 | `3306` |
| `DB_NAME` | 否 | 数据库名 | `mental_health` |
| `DB_USERNAME` | 否 | 数据库用户名 | `root` |
| `DB_PASSWORD` | 是 | 数据库密码 | `root123456` |
| `FLYWAY_ENABLED` | 否 | 启用 Flyway 数据库迁移 | `true` |
| `REDIS_ENABLED` | 否 | 启用 Redis 缓存 | `false` |
| `REDIS_HOST` | 条件 | Redis 主机（Redis 启用时必需） | `localhost` |
| `REDIS_PORT` | 否 | Redis 端口 | `6379` |
| `REDIS_PASSWORD` | 否 | Redis 密码 | 空 |
| `REDIS_DB` | 否 | Redis 数据库编号 | `0` |
| `JWT_SECRET` | 是 | JWT 签名密钥 | 内置默认值 |
| `AI_MOCK_ENABLED` | 否 | AI Mock 模式开关 | `true` |
| `AI_API_KEY` | 条件 | 阿里云百炼 API Key（Mock 关闭时必需） | - |
| `LOGIN_MAX_ATTEMPTS` | 否 | 登录最大尝试次数 | `5` |
| `LOGIN_LOCK_DURATION` | 否 | 登录锁定时长（秒） | `300` |
...
> **安全提示**: 正式环境务必通过环境变量或 `application-local.yml` 覆盖 `DB_PASSWORD`、`JWT_SECRET` 和 `AI_API_KEY`。

---

## 前端说明

<!-- AUTO-GENERATED: page-stats -->
### 页面统计
...
**患者端 (15个页面):**
- Dashboard (工作台) — 统计卡+情绪趋势图+快捷操作
- MoodDiary (情绪日记) — 日记CRUD+天气主题+心情预测
- TimeCapsule (时间胶囊) — 写信/解锁/回信
- TreeHole (心情树洞) — 匿名发布
- RoomDecoration (房间装饰) — 心灵小屋
- Resources (心理资源) — 推荐阅读
- AIChat (AI咨询) — AI问答对话
- Reports (评估报告) — 报告历史
- Assessments (心理评估) — PHQ-9/GAD-7答题向导 ★Phase2
- Meditation (正念冥想) — 7个呼吸/冥想练习 ★Phase3
- Appointments (预约管理) — 预约+等候名单 ★Phase4
- Doctors (医生列表)
- Chat (实时沟通)
- Communication (通讯)
- Profile (个人中心) — 紧急联系人+健康档案 ★Phase1

**医生端 (11个页面):**
- Dashboard (工作台) — 统计+预约趋势+日程
- Patients (患者管理)
- PatientPool (患者公海)
- Reports (评估报告)
- CrisisAlerts (危机告警) — 风险预警+处置 ★Phase3
- Chat (在线咨询)
- Consultation (咨询管理)
- Appointments (预约管理)
- Scheduling (排班设置) — 周排班+特殊日期 ★Phase4
- TreatmentPlans (治疗计划) — 目标/干预/SOAP ★Phase5
- Profile (个人中心)

**管理员端 (8个页面):**
- Dashboard (工作台) — 数据概览+用户趋势
- Users (用户管理) — CRUD+状态管理
- Doctors (医生管理 + 审批)
- Resources (资源管理)
- Appointments (预约管理)
- Messages (消息中心)
...
**v2.0 Phase 1-6 新增功能:**
- [x] Flyway 数据库迁移管理 (8个迁移文件)
- [x] Redis 缓存支持 (可选启用)
- [x] Outbox 异步事件发布 (5秒轮询 + 指数退避重试)
- [x] WebSocket JWT 认证拦截器
- [x] 患者临床档案聚合 (紧急联系人/病史/过敏史)
- [x] 结构化心理评估引擎 (PHQ-9/GAD-7 + 确定性评分DSL)
- [x] 危机干预/风险预警系统 (规则引擎 + SLA升级 + 4小时抗抖动)
- [x] 引导式冥想/呼吸练习 (7个练习 + 呼吸播放器)
- [x] 通知中心 (站内信 + bell+drawer)
- [x] 智能排班系统 (周模板 + 日期覆盖 + 冲突检测 + 等候名单)
- [x] 治疗计划管理 (SMART目标 + CBT等干预 + SOAP记录 + 进度追踪)
- [x] 数据分析与疗效追踪 (疗效快照 + 医生绩效 + 管理员系统分析)
...
1. **POST请求401**: 部分POST API通过Python/curl调用返回401，但前端axios正常（可能与CORS preflight有关）
2. **Vue warnings**: 多页面出现 "Vue received a Component that was made a reactive object" 警告
3. **预约日期选择器**: 大量 "Invalid prop: type check failed" 警告
4. **Dashboard评估计数**: 评估报告统计显示0但实际已有评估记录
...
### v2.0 (2026-03-14) — Phase 1-6 全部完成

**Phase 1: 平台基础设施**
- Flyway 数据库迁移 (8个版本化SQL)
- Redis 缓存框架 (可选启用)
- Outbox 异步事件模式 (5秒轮询 + 指数退避)
- WebSocket JWT 认证拦截器
- 患者临床档案聚合 (紧急联系人/病史/过敏史)

**Phase 2: 结构化心理评估引擎**
- PHQ-9/GAD-7 标准化量表 + 种子数据
- 确定性评分DSL引擎 (SUM/MAP_RANGE)
- 多步答题向导 + 自动保存 + 结果解读
- 医生查看患者评估历史

**Phase 3: 危机干预/风险预警**
- 8条风险规则 (关键词/PHQ9_Q9/趋势匹配)
- 风险评分服务 + 4小时抗抖动告警
- SLA自动升级调度器 (5分钟轮询)
- 医生告警页面 + 处置记录
- 患者安全资源API (3条热线 + 2个自助工具)
- 冥想模块: 7个引导练习 + 呼吸播放器
- 通知中心: bell + drawer
...
**Phase 5: 治疗计划管理**
- 计划全生命周期 (DRAFT→ACTIVE→COMPLETED)
- SMART目标管理 + 进度追踪
- 干预措施 (CBT/药物/练习等7类)
- SOAP咨询记录
...
**Bug修复:**
- 管理员用户状态显示bug (`...user` spread覆盖计算字段)
- 患者预约医生下拉空白 (IPage对象未取.records)

### v1.3 (2026-02-23)

**功能完善与体验优化:**
- 新增忘记密码页面 (ForgotPassword)
- 新增 JWT 过期检测工具 (`utils/jwt.ts`)
- 新增时间格式化工具 (`utils/time.ts`)
- 新增共享组合式函数 (`composables/useShared.ts`)
- 重构 API 模块：患者端（日记/评论/树洞/胶囊/聊天/AI/资源/房间/报告）、医生端、管理员端
- 重构大型页面为组合式函数模式（MoodDiary、TreeHole、Profile、Appointments、Messages、Doctors）
- 优化路由守卫：增加 Token 过期自动登出
- 优化请求拦截器
- 后端新增 AuthService 密码重置支持

### v1.2 (2026-01-26)

**UI 主题升级 - 温暖阳光风格:**
- 全局主题样式重构（theme.scss、global.scss、element-override.scss）
- 三套布局组件视觉升级（PatientLayout、DoctorLayout、AdminLayout）
- 登录/注册页面 UI 优化
- 患者工作台 Dashboard 重新设计

### v1.1 (2026-01-22)

**清理冗余文档，优化项目结构:**
- 移除 36 个冗余文档文件（开发日志、阶段总结等）
- 整合项目文档至 `docs/README.md`
- 新增数据库修复脚本 (`sql/ensure-doctor-data.sql`)
- 代码净减 ~11000 行
