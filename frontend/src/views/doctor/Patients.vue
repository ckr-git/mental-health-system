<template>
  <div class="patients-container">
    ...
    <el-dialog v-model="detailVisible" title="患者详情" width="80%" top="5vh">
      <el-tabs v-if="currentPatient" v-model="detailTab">
        ...
        <el-tab-pane label="评估历史" name="assessments">
          <el-table :data="patientAssessments" v-loading="assessmentsLoading" style="width: 100%">
            <el-table-column prop="scaleName" label="量表" min-width="140" />
            <el-table-column prop="totalScore" label="总分" width="80" align="center" />
            <el-table-column label="严重程度" width="120" align="center">
              <template #default="{ row }">
                <el-tag :type="getAssessmentTagType(row.severityLevel)" size="small">
                  {{ row.severityLevel }}
                </el-tag>
              </template>
            </el-table-column>
          ...
          <el-empty v-if="!assessmentsLoading && patientAssessments.length === 0" description="暂无评估记录" />
        </el-tab-pane>
      </el-tabs>
    </el-dialog>
...
import type { User, AssessmentHistoryItem } from '@/types'
...
const patientAssessments = ref<AssessmentHistoryItem[]>([])
...
const viewDetail = async (patient: any) => {
  try {
    ...
    if (res.code === 200 && res.data) {
      ...
      loadPatientAssessments(patient.id)
    }
  } catch (error) {
...
const loadPatientAssessments = async (patientId: number) => {
  assessmentsLoading.value = true
  patientAssessments.value = []
  try {
    const res = await assessmentApi.getPatientAssessments(patientId)
    if (res.code === 200 && res.data) {
      patientAssessments.value = res.data.records
    }
  } catch (error) {
    console.error('Failed to load patient assessments:', error)
  } finally {
    assessmentsLoading.value = false
  }
}

const getAssessmentTagType = (level: string) => {
  const map: Record<string, string> = {
